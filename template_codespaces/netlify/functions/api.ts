...
console.log("Supabase URL cargada:", SUPABASE_URL ? "SÍ" : "NO");
console.log("Supabase Key cargada:", SUPABASE_SERVICE_KEY ? "SÍ" : "NO")
...
app.get("/api/offers/:pda", async (request, reply) => {
...
app.delete("/api/offers/:pda", async (request, reply) => {
  const wallet = getWalletFromToken(request.headers.authorization);
  if (!wallet) return reply.code(401).send({ error: "Unauthorized" });
...
// -- Dashboard --
app.get("/api/merchant/transactions", async (request, reply) => {
  const wallet = getWalletFromToken(request.headers.authorization);
  if (!wallet) return reply.code(401).send({ error: "Unauthorized" });
...
// ─── Exportar para Netlify Serverless ─────────────────────────────────────────

// Envolvemos nuestra app de Fastify con el adaptador
const proxy = awsLambdaFastify(app);

// Netlify busca exactamente esta función "handler" exportada
